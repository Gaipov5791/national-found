...
export function NewsPreviewCard({
  ...
  return (
    ...
    >
      <div className="flex aspect-[4/3] min-h-[5.5rem] items-center justify-center bg-white/15 font-display text-xs tracking-wide text-white/60 sm:min-h-[6.5rem] sm:text-sm">
        {resolvedImageLabel}
      </div>
      {title ? (
        <h3 className="px-3 pt-2.5 font-display text-sm font-semibold leading-snug tracking-tight text-white sm:px-4 sm:pt-3 sm:text-[15px]">
          {title}
        </h3>
      ) : null}
      <p
        className={cn(
          ...
          title ? "pb-2.5 pt-1 sm:pb-3 sm:pt-1.5" : "py-2.5 sm:py-3"
        )}
      >
        {resolvedDescription}
      </p>
