...
function cloudInfluence(y: number): CursorBg | null {
  ...
  const opacity = parseFloat(getComputedStyle(cloud).opacity);
  if (opacity < 0.12) return null;

  const rect = cloud.getBoundingClientRect();
  const fadeEnd = rect.top + rect.height * 0.92;
...
function detectBackgroundAt(x: number, y: number): CursorBg {
  ...
  const cloudOpacity = cloudEl ? parseFloat(getComputedStyle(cloudEl).opacity) : 0;
